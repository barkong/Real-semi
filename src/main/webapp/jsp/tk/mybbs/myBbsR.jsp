				...
				<c:when test="${mbr ne null }">
					<table class="bbsTable">
						...
						<tbody class="bsstbody">
							<c:forEach var="mb" items="${mbr }">
								<c:if test="${mb.mbr_cat ne null }">
									<tr>
										<td>${mb.mbr_cat }</td>
										<td>${mb.mbr_no }</td>
										<td class="bbsTable_title"><a
											href="ReviewDetailC?no=${mb.mbr_no }">${mb.mbr_title }</a></td>
										<td>${mb.mbr_id }</td>
					...
					<div class="bbsPaging">
						<span><c:choose>
								...
								<c:otherwise>
									<a href="MyBbsPageRC?p=${curPageNo - 1 }"> ◀이전 </a>
								</c:otherwise>
							</c:choose></span> <a href="MyBbsPageRC?p=1">[맨처음]</a>
						<c:forEach var="i" begin="1" end="${pageCount }">
							<a href="MyBbsPageRC?p=${i }"> [${i }] </a>
						</c:forEach>
						<span><a href="MyBbsPageRC?p=${pageCount }">[맨끝]</a></span> <span><c:choose>
								<c:when test="${curPageNo == pageCount}">
								</c:when>
								<c:otherwise>
									<a href="MyBbsPageRC?p=${curPageNo + 1 }"> 다음▶ </a>
								</c:otherwise>
							</c:choose></span>
					</div>
				</c:when>
